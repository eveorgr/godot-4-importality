...
func get_import_metadata(source_file: String) -> Dictionary:
	var archive := ZIPReader.new()
	if archive.open(ProjectSettings.globalize_path(source_file)) != OK:
		return {}
	...
	var rules: Variant = manifest.get("visibility_rules", {})
	if not (rules is Dictionary):
		return {}
	return {VISIBILITY_META_KEY: rules}
...
func _export(res_source_file_path: String, options: Dictionary) -> ExportResult:
	...
	for slot_value: Variant in manifest["slots"]:
		...
		for variant_value: Variant in slot["variants"]:
			...
			animation_specs.append({"name": animation_name, "frames": frame_specs, "direction": _direction_from_manifest(str(playback["direction"])), "repeat_count": int(float(playback["repeat_count"]))})
	archive.close()
	if images.is_empty():
...
func _read_manifest(archive: ZIPReader) -> Dictionary:
	var buffer := archive.read_file("manifest.json")
	if buffer.is_empty(): return {"error": ERR_INVALID_DATA, "message": "Krita layer bundle has no manifest.json"}
	var parsed: Variant = JSON.parse_string(buffer.get_string_from_utf8())
	if not (parsed is Dictionary): return {"error": ERR_PARSE_ERROR, "message": "Krita layer manifest is not a JSON object"}
	var manifest: Dictionary = parsed
	var validation_error := _validate_manifest(manifest)
	if not validation_error.is_empty(): return {"error": ERR_INVALID_DATA, "message": validation_error}
...
func _read_frame(archive: ZIPReader, frame: Dictionary, canvas_size: Vector2i) -> Dictionary:
	...
	if not _is_safe_archive_path(archive_file): return {"error": ERR_INVALID_DATA, "message": "Unsafe frame archive path: %s" % archive_file}
	var png_buffer := archive.read_file(archive_file)
	if png_buffer.is_empty(): return {"error": ERR_FILE_NOT_FOUND, "message": "Missing frame: %s" % archive_file}
	if _sha256_buffer(png_buffer) != str(frame["sha256"]).to_lower(): return {"error": ERR_FILE_CORRUPT, "message": "Frame SHA-256 mismatch: %s" % archive_file}
	var image := Image.new()
	var image_error := image.load_png_from_buffer(png_buffer)
	if image_error != OK: return {"error": image_error, "message": "Failed to decode %s: %s" % [archive_file, error_string(image_error)]}
	if image.get_size() != canvas_size: return {"error": ERR_INVALID_DATA, "message": "Frame %s is %s, expected %s" % [archive_file, image.get_size(), canvas_size]}
	return {"error": OK, "message": "", "image": image}

func _validate_manifest(manifest: Dictionary) -> String:
	if str(manifest.get("schema", "")) != SCHEMA: return "Unsupported Krita layer bundle schema: %s" % manifest.get("schema", null)
	if not (manifest.get("canvas") is Dictionary): return "Krita layer manifest has no canvas object"
	var canvas: Dictionary = manifest["canvas"]
	if int(canvas.get("width", 0)) <= 0 or int(canvas.get("height", 0)) <= 0: return "Krita layer canvas size must be positive"
	if not (manifest.get("animation") is Dictionary): return "Krita layer manifest has no animation object"
	if float((manifest["animation"] as Dictionary).get("fps", 0.0)) <= 0.0: return "Krita layer manifest FPS must be positive"
	if not (manifest.get("slots") is Array) or (manifest["slots"] as Array).is_empty(): return "Krita layer manifest slots must be a non-empty array"
	...
	for slot_value: Variant in manifest["slots"]:
		...
		for variant_value: Variant in slot["variants"]:
			...
			if str(variant.get("blend_mode", "")).to_lower() != "normal" or int(variant.get("opacity", -1)) != 255: return "Top-level exported variants must use normal blend and opacity 255"
			...
			if is_nan(repeat_number) or is_inf(repeat_number) or repeat_number < 0.0 or not is_equal_approx(repeat_number, floor(repeat_number)): return "repeat_count must be a non-negative integer-valued JSON number"
